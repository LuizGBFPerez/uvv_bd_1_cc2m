DROP DATABASE IF EXISTS sistemacongresso;
DROP ROLE IF EXISTS gruposeis;

CREATE ROLE gruposeis WITH
  NOSUPERUSER
  CREATEDB
  CREATEROLE
  LOGIN
  ENCRYPTED PASSWORD '654321';

CREATE DATABASE sistemacongresso WITH
  owner      = gruposeis
  template   = template0
  encoding   = 'UTF-8'
...
CREATE SCHEMA IF NOT EXISTS projeto AUTHORIZATION gruposeis;

ALTER ROLE gruposeis SET SEARCH_PATH TO projeto, "$user", public;

SET SEARCH_PATH TO projeto, "$user", public;
